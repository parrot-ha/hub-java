...
public class DeviceJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        String handlerMethod = context.getJobDetail().getKey().getName();

        String jobKeyGroup = context.getJobDetail().getKey().getGroup();
        String deviceId = "";
        if(jobKeyGroup.startsWith(ScheduleService.DEVICE_TYPE + ":")) {
            deviceId = jobKeyGroup.substring(4);
        } else {
            deviceId = jobKeyGroup;
        }

        ServiceFactory.getEntityService().runDeviceMethod(deviceId, handlerMethod);
    }
}
